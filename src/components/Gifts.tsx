  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto space-y-8 sm:space-y-10">
        ...
        {/* Action Button: DATOS BANCARIOS & Discreet Venue Note */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8, delay: 0.3 }}
          className="flex flex-col items-center justify-center pt-2 space-y-6"
        >
          <button
            onClick={() => setBankModalOpen(true)}
            className="w-full sm:w-auto inline-flex items-center justify-center px-9 py-4 rounded-full bg-[#2A221E] text-[#FAF7F2] border border-[#2A221E] hover:border-[#656D4A]/50 shadow-[0_10px_25px_rgba(42,34,30,0.08)] hover:shadow-[0_14px_32px_rgba(42,34,30,0.15)] hover:bg-[#3D322C] active:scale-[0.99] transition-all duration-300 text-xs font-sans uppercase tracking-[0.25em] font-medium cursor-pointer"
          >
            <span>DATOS BANCARIOS</span>
          </button>

          <p className="font-sans text-xs sm:text-sm text-[#9C9286] font-light tracking-wide">
            También encontrarás un buzón en el salón.
          </p>
        </motion.div>
      </div>
        ...
        {bankModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-[#2A221E]/40 backdrop-blur-sm">
            <motion.div
              ...
              className="relative w-full max-w-md bg-[#FFFFFF] rounded-2xl p-8 sm:p-10 border border-[#E8E2D8] shadow-2xl text-left"
            >
              {/* Close Button */}
              <button
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3 border-b border-[#E8E2D8] pb-4">
                  ...
                  <h3 className="font-serif text-2xl sm:text-3xl font-light text-[#2A221E]">
                    Datos Bancarios
                  </h3>
                </div>
                ...
                <div className="space-y-4 text-sm font-sans">
                  {/* Alias Box */}
                  <div className="p-5 rounded-xl bg-[#FAF7F2] border border-[#E8E2D8] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-1">
                      <p className="text-xs uppercase tracking-wider text-[#9C9286] font-medium">Alias</p>
                      <p className="font-mono text-base sm:text-lg font-semibold text-[#2A221E] tracking-wider select-all">
                        {alias}
                      </p>
                    </div>
                    <button
                      onClick={copyAlias}
                      className="px-5 py-2.5 rounded-full bg-[#2A221E] text-[#FAF7F2] hover:bg-[#3D322C] transition-colors flex items-center justify-center gap-2 cursor-pointer text-xs font-medium tracking-wider uppercase shadow-sm shrink-0"
                    >
                      {copiedAlias ? (
                        <>
                          <Check className="w-3.5 h-3.5 text-[#656D4A]" />
